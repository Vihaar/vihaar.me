...
const FluidEmailReveal: React.FC<FluidEmailRevealProps> = ({
  ...
  useEffect(() => {
    ...
    const createBubbles = () => {
      ...
      for (let i = 0; i < bubbleCount; i++) {
        ...
        const bubble: Bubble = {
          ...
          // More opaque to better hide email
          vx: 0,
          vy: 0
        };
        bubbles.push(bubble);
    ...
    const getRandomColor = (alpha: number) => {
      const colors = [`rgba(255, 99, 132, ${alpha})`,
      // Red
      ...
      // Orange
      `rgba(255, 127, 223, ${alpha})`,
      // Pink
      `rgba(102, 204, 153, ${alpha})`,
      // Mint
      `rgba(178, 102, 255, ${alpha})`,
      // Lavender
    ...
    function animate() {
      ...
      for (let i = 0; i < bubbles.length; i++) {
        ...
        const bubbleGradient = ctx.createRadialGradient(bubble.x - bubble.radius * 0.3, bubble.y - bubble.radius * 0.3, bubble.radius * 0.1, bubble.x, bubble.y, bubble.radius);
  ...
  return <div ref={containerRef} className={`relative rounded-lg overflow-hidden ${className}`}>
      <canvas ref={canvasRef} className="w-full h-full z-10r" />
      <div ref={emailTextRef} className="absolute z-0 text-center font-bold text-2xl select-all">
        {email}
      </div>
    </div>;
};
